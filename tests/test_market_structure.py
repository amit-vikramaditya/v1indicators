...
def test_market_structure_basic():
    ...
    window = 3
    # Causal default: a pivot at bar t is only knowable at t + right, so the
    # expected markers/levels are the legacy pivots delayed by `right` bars.
    pivot_high = high.where(high == high.rolling(window).max().shift(-1)).shift(1)
    pivot_low = low.where(low == low.rolling(window).min().shift(-1)).shift(1)
...
def test_market_structure_retrospective_mode():
    """causal=False restores the legacy pivot-at-own-bar placement."""
    high = pd.Series([10.0, 11.0, 12.0, 11.0, 13.0, 12.0, 14.0, 13.0, 12.0])
    low = pd.Series([9.0, 10.0, 11.0, 10.0, 11.0, 10.0, 12.0, 11.0, 10.5])
    close = pd.Series([9.5, 10.8, 11.7, 10.2, 12.6, 10.1, 13.7, 10.9, 10.2])

    result = market_structure(high, low, close, left=1, right=1, causal=False)
